...
| Bot | Interactúa con el _API_ pública de la aplicación para obtener información de jugadores/partidas y para jugar contra la máquina mediante el método _play_
| Aplicación web YOVI | Muestra la interfaz web al usuario y expone el API público para los bots.
|===
...
La aplicación se divide en tres servicios: el motor de juego, _Gamey_, el módulo de gestión de usuarios, _Users_, y la interfaz de la aplicación web, _Webapp_. _Webapp_ se comunica con _Gamey_ y _Users_ para poder llevar a cabo sus funciones de gestionar sesiones de usuarios y estadísticas. A su vez, _Users_ se conecta con la base de datos para obtener la información relativa a los usuarios y estadísticas.
...
El módulo de juego _Gamey_ separa las responsabilidades de gestión de peticiones de juego, gestión de mecánicas del juego y selección de IAs contra las que jugar en submódulos diferentes con el objetivo de que estos sean fácilmente configurables o reemplazables.
...
|===
| **Nombre** | **Responsabilidad**
| service/users | Gestiona el registro e inicio de sesión de usuarios
| service/stats | Gestiona las estadísticas de juego y rankings de usuarios
|===
...
node "Aplicación web YOVI" as yovi {
    ...
    component Webapp #DarkGray
    component Users {
        component "service/users" as users #DarkGray
        component "service/stats" as stats #DarkGray
    }
    component BD as db{
