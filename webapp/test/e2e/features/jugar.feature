Feature: Jugar una partida
  ...
  Scenario: Iniciar una partida correctamente
    Given Me he registrado con nombre "Prueba", usuario "_usuario_test_" y contraseña "test123..." y accedo al lobby
  ...
  Scenario: Hacer un movimiento en el tablero
    Given Estoy en una partida en curso como "_usuario_test_"
  ...
  Scenario: Detectar la victoria del jugador
    Given Estoy en una partida en curso como "_usuario_test_" con el servidor de juego simulado que devuelve victoria
    When Hago clic en una casilla vacía del tablero
