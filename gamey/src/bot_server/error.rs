...
pub struct ErrorResponse {
    /// A machine-readable error code for client-side translation.
    pub code: String,
    ...
    pub message: String,
    /// Optional technical details that are useful for debugging.
...
impl ErrorResponse {
    ...
    /// * `message` - A user-facing description of the error
...
mod tests {
    ...
    fn test_error_with_no_context() {
        let err = ErrorResponse::error("GENERIC_ERROR", "Generic error", None, None);
    ...
    fn test_error_with_partial_context() {
        let err = ErrorResponse::error("VERSION_ERROR", "Version error", Some("v2".to_string()), None);
        assert_eq!(err.code, "VERSION_ERROR");
    ...
    #[test]
    fn test_deserialize() {
        let json = r#"{"code":"TEST","api_version":"v1","bot_id":"random","message":"error msg","details":null}"#;
    ...
    #[test]
    fn test_clone() {
        let err = ErrorResponse::error("CLONE_TEST", "Clone test", Some("v1".to_string()), None);
